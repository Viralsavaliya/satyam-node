...
const register = (user_data, userType, otp) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let payload = yield aes_1.default.encrypt(uniqueUserKey, config.get("OUTER_KEY_PAYLOAD"));
...
const logout = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    jwt.verify(token, config.get("JWT_ACCESS_SECRET"), (err, user) => __awaiter(void 0, void 0, void 0, function* () {
        if (err) {
            return commonUtils_1.default.sendError(req, res, { message: appStrings_1.AppStrings.INVALID_TOKEN }, 401);
        }
        else {
            const uniqueUserKey = aes_1.default.decrypt(user.sub, config.get("OUTER_KEY_PAYLOAD"));
...
const getAccessTokenPromise = (oldToken) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
        jwt.verify(oldToken, config.get("JWT_REFRESH_SECRET"), (err, user) => __awaiter(void 0, void 0, void 0, function* () {
            if (err) {
                ...
                let payload = aes_1.default.encrypt(uniqueUserKey.toString(), config.get("OUTER_KEY_PAYLOAD"));
                const accessToken = jwt.sign({ sub: payload }, config.get("JWT_ACCESS_SECRET"), { expiresIn: config.get("JWT_ACCESS_TIME") });
                const refreshToken = yield generateRefreshToken(payload);
                let data = { accessToken: accessToken, refreshToken: refreshToken };
...
const _generateRefreshToken = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    return Jwt.sign({ sub: payload, aud: models_1.UserTokenRole.refreshToken }, config.get("JWT_ACCESS_SECRET"), { expiresIn: config.get("JWT_REFRESH_TIME") });
});
const encryptPayload = (data) => {
    let encryptedData = aes_1.default.encrypt(JSON.stringify(data), config.get("OUTER_KEY_USER"));
    let payload = aes_1.default.encrypt(encryptedData, config.get("OUTER_KEY_PAYLOAD"));
    return { data: encryptedData, payload: payload };
};
const decryptPayload = (payload) => {
    let decryptedPayload = aes_1.default.decrypt(payload, config.get("OUTER_KEY_PAYLOAD"));
    let decryptedData = aes_1.default.decrypt(decryptedPayload, config.get("OUTER_KEY_USER"));
